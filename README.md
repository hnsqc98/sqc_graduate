...
### 2020/04/20
* 确定了论文大纲，并开始写作。
* 完成了HoloLens应用内的的英文语音识别、中文语音识别。
* 可以做到在应用内说“变化”、“开启”、“关闭”等词语，Hololens可以识别并进行相应的我设定好的指令，比如变化颜色，显示/隐藏物体等。
* 现有问题：整合的时候，将语音识别作为分界面并入主界面后，在主界面点击跳转，转入分界面后语音识别功能失效。不知道这是为什么，我也已经把语音识别的全局存在给打开了，但是跳转后仍然不识别；作为对照，我单独部署了一个语音识别的应用，在Hololens上运行是没有问题的；也就是说明并入主界面这步操作有问题，但我检查了几天都没有解决哪里有问题。还要再看。

### 2020/04/20
* 解决了上个星期的跳转不识别问题，大致原因是MRTK在跳转过程中自动屏蔽了分场景的自定义设置，导致语音关键词识别出不来，而后在整体场景中重新设置若干项后得以解决。
* 计划单独放一个“Speaking Training”模块，专门用来进行语言的训练
* 接下来就是完成各个分场景的语音引导，打算先计算机生成语音。
* 分场景的进一步完善
* 论文进行了大致调研，参考类似论文的思路，进行进一步写作。
